...
class maquina : AppCompatActivity() {
    ...
    fun buttonClick(view: View) {
        ...
        Handler(Looper.getMainLooper()).postDelayed({
            when (posicao) {
                0 -> {
                    binding.buttonUm.setBackgroundColor(Color.BLUE)
                    binding.buttonUm.isEnabled = false
                }

                1 -> {
                    binding.buttonDois.setBackgroundColor(Color.BLUE)
                    binding.buttonDois.isEnabled = false
                }

                2 -> {
                    binding.buttonTres.setBackgroundColor(Color.BLUE)
                    binding.buttonTres.isEnabled = false
                }

                3 -> {
                    binding.buttonQuatro.setBackgroundColor(Color.BLUE)
                    binding.buttonQuatro.isEnabled = false
                }

                4 -> {
                    binding.buttonCinco.setBackgroundColor(Color.BLUE)
                    binding.buttonCinco.isEnabled = false
                }

                5 -> {
                    binding.buttonSeis.setBackgroundColor(Color.BLUE)
                    binding.buttonSeis.isEnabled = false
                }

                6 -> {
                    binding.buttonSete.setBackgroundColor(Color.BLUE)
                    binding.buttonSete.isEnabled = false
                }

                7 -> {
                    binding.buttonOito.setBackgroundColor(Color.BLUE)
                    binding.buttonOito.isEnabled = false
                }

                8 -> {
                    binding.buttonNove.setBackgroundColor(Color.BLUE)
        ...
        if (jogadorRecente.equals("X")) {
            buttonEscolhido.setBackgroundResource(R.drawable.estrela)
            jogadorRecente = "O"
        } else {
            buttonEscolhido.setBackgroundResource(R.drawable.tartaruga)
            jogadorRecente = "X"
        }
